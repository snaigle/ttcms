...
public class NewsController {
	 ...
	 */
	@At({ "/tag/*", "/tag" })
	public Object listByTag(@Param("id") int id, @Param("offset") int offset,
			@Param("max") int max) {
		if (id == 0) {
			return CV.redirect("/news/list", "标签不能为空");
		}
		PageForm<News> pf = PageForm
				.getPaper(
						dao,
						News.class,
						Cnd.format(
								"id in (select news_id from t_news_tag where tag_id = %d) order by id desc",
								id),
						Cnd.format(
								"id in (select news_id from t_news_tag where tag_id = %d)",
								id), offset, max);
		for (News news : pf.getResults()) {
			dao.fetchLinks(news, null);
		}
	 ...
	 */
	@At({ "/month/*", "/month" })
	public Object listByMonth(@Param("month") String month,
			@Param("offset") int offset, @Param("max") int max) {
		if (Strings.isEmpty(month)) {
			return CV.redirect("/", "日期归档不能为空");
		}
		PageForm<News> pf = PageForm.getPaper(
				dao,
				News.class,
				Cnd.where("concat(year(create_time),'-',month(create_time))",
						"=", month).desc("id"), Cnd.where(
	 ...
	 */
	@At({ "/cat/*", "/cat" })
	public Object listByCategory(@Param("id") int id,
			@Param("offset") int offset, @Param("max") int max) {
		if (id == 0) {
		...
		PageForm<News> pf = PageForm
				.getPaper(
						...
						News.class,
						Cnd.format(
								"id in (select news_id from t_news_category where category_id = %d) order by id desc",
								id),
						Cnd.format(
								"id in (select news_id from t_news_category where category_id = %d)",
								id), offset, max);
		for (News news : pf.getResults()) {
			dao.fetchLinks(news, null);
		}
	 ...
	 */
	@At({ "/search/*", "/search" })
	public Object search(@Param("p") String p, @Param("offset") int offset,
			@Param("max") int max) {
		...
		PageForm<News> pf = PageForm.getPaper(
				dao,
				News.class,
				Cnd.where("title", "like", "%" + p + "%")
						.or("content", "like", "%" + p + "%").desc("id"),
				Cnd.where("title", "like", "%" + p + "%").or("content", "like",
						"%" + p + "%"), offset, max);
		for (News news : pf.getResults()) {
			dao.fetchLinks(news, null);
		}
	...
	@Ok("raw")
	@POST
	public Object saveComment(HttpSession session, HttpServletRequest req,
			@Param("username") String username, @Param("code") String code,
			@Param("content") String content, @Param("newsId") long newsId) {
		...
		if (session == null
				|| session.getAttribute("verifyCode") == null
